...
> 전체 10건. `clean_news` 중 요약 완료 레코드 전수.

---

### 3-2. 압축률 패턴 분석

#### (A) 요약 길이는 왜 188~200자로 수렴하는가

```
[관찰]  요약 길이 표준편차 = 4.1자 (188~200자 범위)
[원인]  summarizer.py: max_chars = 200 (config 기본값)
        _clean_summary() 후 truncate(max_chars * 2) 적용
        → 모델이 "200자 이내" 제약을 지키면서 최대한 채우는 경향
[결론]  요약기는 사실상 "고정 길이 ~200자" 출력기로 동작
```

#### (B) 본문 길이와 압축률의 관계

```
[관찰]  본문 2,876자 → 압축률 7.0%  (정보 밀도 ↑)
        본문   194자 → 압축률 100%  (압축 효과 없음)
[원인]  요약 길이가 고정(~200자)이므로
        압축률 = 200 / 본문길이 에 수렴
        → 본문이 짧을수록 압축률이 100%에 근접
[결론]  압축 효과의 손익분기점 ≈ 200자 (본문 = 요약 길이)
        실질적 압축 효과 구간: 본문 ≥ 400자 (압축률 ≤ 50%)
        권장 수집 기준:        본문 ≥ 300자 (압축률 ≤ 67%)
```

#### (C) id=2 — 압축률 100%의 의미

```
[관찰]  id=2: 본문 194자, 요약 194자
[원인]  content_fallback_to_description: true 설정으로
        본문 파싱 실패 → RSS description(194자)으로 폴백
        min_content_length: 60 기준은 통과 (194 > 60)
        → 정제 통과 + 요약 API 호출 + 결과: 원문 그대로 반환
[결론]  "정제 통과 ≠ 요약 품질 보장"
        두 설정의 상호작용이 만든 사각지대:
        60자 < 본문 < 300자 구간이 비효율 호출 구간
```
...
```
max_input = 4,000자 (config 기본값)
실측 최장 본문 = 2,876자 → truncate 미발생 (여유 1,124자)

입력 토큰 추정 (한글 1자 ≈ 2~3토큰):
  본문 2,876자 × 2.5 ≈ 7,190 토큰
  프롬프트 오버헤드(USER_PROMPT 템플릿) ≈ 80 토큰
  SYSTEM_PROMPT ≈ 60 토큰
  총 입력 ≈ 7,330 토큰 (최대 케이스)

  본문 194자(id=2) × 2.5 ≈ 485 토큰
  총 입력 ≈ 625 토큰 (최소 케이스)
```
...
```
max_tokens = max(256, max_chars * 3) = max(256, 600) = 600 토큰 (기본값)
실측 요약 길이 ≈ 194자 × 2.5 ≈ 485 토큰
→ 할당 600 토큰 중 실사용 ≈ 485 토큰 (약 81% 활용)
```

#### 비효율 호출 식별

```
id=2: 입력 625토큰 + 출력 485토큰 = 1,110토큰 소비
      압축률 100% → 정보 이득 = 0
      → 1,110토큰 낭비 (API 비용 기준 약 $0.0003~0.001/건)
```
...
```python
# summarizer.py run() — 현재 코드 (스킵 없음)
content = row["content"] or ""
if not content.strip():
    stats.failed += 1
    log.warning("[%d/%d] ID=%d 본문이 비어 있어 스킵", ...)
    continue
```
...
if len(content) < MIN_CONTENT_FOR_SUMMARY:
    ...
    # 원문을 요약란에 그대로 저장하거나 빈 채로 유지
    continue
```

> **효과:** id=2 케이스(194자)에서 API 호출 1건 절감.
> 300자 미만 기사 비율이 10%라면 전체 비용의 10% 절감 가능.

#### 방안 B — `max_input` 동적 조정

```python
# 현재: 모든 기사에 동일한 max_input=4,000자 적용
body = truncate(content, self.max_input, suffix="")

# 개선안: 짧은 본문은 max_input을 본문 길이로 제한
effective_max = min(self.max_input, max(len(content), 500))
body = truncate(content, effective_max, suffix="")
```

> **효과:** 짧은 본문에서 불필요한 프롬프트 패딩 방지.
> 실측상 효과는 미미 (본문이 이미 짧으면 truncate 미발생).

#### 방안 C — 압축률 100% 경고 지표 추가

```python
# report.py — 개선안
inefficient = [r for r in summarized if len(r["summary"]) >= len(r["content"]) * 0.95]
if inefficient:
    log.warning(
        "압축률 95%% 이상(사실상 미압축) 기사 %d건: ID=%s",
        len(inefficient),
...
| 관찰 | 원인 | 최적화 방안 | 예상 효과 |
|------|------|-------------|-----------|
| 요약 길이 고정 ~200자 | `max_chars=200` 제약 | — (의도된 동작) | — |
| 본문 194자 → 압축률 100% | `min_content_length=60` 사각지대 | 방안 A: 요약 스킵 기준 300자 추가 | API 호출 절감 |
| `max_input=4,000자` 여유 과다 | 실측 최장 2,876자 | 방안 B: 동적 조정 | 미미 |
| 압축률 100% 감지 불가 | 리포트 지표 없음 | 방안 C: 경고 지표 추가 | 품질 모니터링 |
| 입력 토큰 최대 7,330 | 긴 본문 + 프롬프트 오버헤드 | `max_input` 유지 (비용 대비 품질 균형) | — |

> **핵심 교훈:**
> `min_content_length(60)` 은 **정제 품질 기준**이고,
> `min_content_for_summary(300)` 은 **요약 비용 효율 기준**이다.
> 두 기준은 목적이 달라 **독립적으로 설정**해야 한다.
